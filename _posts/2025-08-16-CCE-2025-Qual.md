...
다만, `node-sql-parser` 라이브러리를 통해 SQL 쿼리문을 파싱한 후, 허용된 SQL 구문, Column 값 등 검증 과정을 거칩니다.

이후, 검증이 모두 완료되면 `executeQuery(query)` 함수를 실행하여 쿼리문을 실행합니다.

하지만, SQL 파싱 로직과 쿼리문을 실행하는 로직에서 처리하는 방법이 달라 우회가 발생합니다.

`created_at ASC--; xxxxx--` 쿼리문을 전달하게 되면, `created_at ASC--;` 해당 부분까지 파싱되고, `xxxxx` 부분은 SQL 구문 검증에 해당되지 않게 됩니다.

또한, `executeQuery(query)` 함수는 `spawn()` 함수를 사용해 스트림 방식으로 쿼리문을 처리합니다. 이로 인해, 이후 원하는 쿼리문을 실행시킬 수 있게 되는 문제가 발생합니다.
