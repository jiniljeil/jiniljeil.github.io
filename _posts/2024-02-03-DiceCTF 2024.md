...
반면, 위와 같이 `object` 타입을 인자로 넘겨 `value` 값을 변경하고 리턴해주면 등록된 ESLint Rule을 우회하여 XSS 공격이 가능하다.    
...
다른 방법으로 CSS에 변수 여러 개를 선언하여 로딩 타임을 증가시켜 타임 기반으로 토큰을 알아내는 방법이 존재한다.                     
...
bot이 페이지에 방문하였을 때, 토큰의 prefix 값이 매칭된다면 변수로 인해 로딩 타임이 길어지게 되어 bot의 브라우저가 닫히지 않은 상태로 유지된다. 그 때, 다시 요청을 보내면 `already open!`를 반환한다.       

즉, 요청을 보내고 bot이 CSS 적용 중일 때 다시 요청을 보냈을 때, `already open!`를 반환한다면 토큰의 prefix 값이 일치한다는 것을 알 수 있다.      
